<h1 align="center">Olá, eu sou o Brendon 👋</h1>
<h3 align="center">Analista de Dados | Business Intelligence & Analytics</h3>

<p align="center">
  <img src="https://img.shields.io/badge/-Brasil-009739?style=flat&logo=googlemaps&logoColor=white" alt="Localização" />
  <img src="https://img.shields.io/badge/Status-Aberto%20a%20oportunidades-success?style=flat" alt="Status" />
</p>

<p align="center">
  📊 Transformo dados brutos em decisões de negócio através de Business Intelligence, modelagem de dados e storytelling executivo.
</p>

---

### 🧑‍💻 Sobre mim

- 🎓 Bacharel em Sistemas de Informação
- 📈 Foco em **modelagem de dados**, **construção de KPIs** e **dashboards executivos**
- 🧩 Gosto de transformar problemas de negócio em modelos analíticos (Star Schema, DAX, métricas de impacto financeiro)
- 🌱 Em constante evolução: estudando SQL avançado, Python para análise de dados e boas práticas de visualização
- 🎯 Em transição de carreira para a área de dados, construindo este portfólio como prova prática das minhas competências

---

### 🛠️ Tecnologias e ferramentas

<p align="left">
  <img src="https://img.icons8.com/?size=50&id=Ny0t2MYrJ70p&format=png&color=000000" alt="Power BI"/>
  <img src="https://img.icons8.com/?size=50&id=J6KcaRLsTgpZ&format=png&color=000000" alt="SQL Server"/>
  <img src="https://img.icons8.com/?size=50&id=13441&format=png&color=000000" alt="Python"/>
  <img src="https://img.icons8.com/?size=50&id=117561&format=png&color=000000" alt="Excel"/>
  <img src="https://img.icons8.com/?size=50&id=zfHRZ6i1Wg0U&format=png&color=000000" alt="Figma"/>
  <img src="https://img.icons8.com/?size=50&id=62856&format=png&color=000000" alt="Git"/>
</p>

---

### 📬 Contato

<p align="left">
  <a href="https://www.linkedin.com/in/brendongomessantos/" target="_blank">
    <img src="https://img.icons8.com/?size=50&id=13930&format=png&color=000000" alt="LinkedIn"/>
  </a>
</p>

---
### 📌 Projetos em Destaque

**[👥 People Analytics — Diagnóstico Estratégico de Turnover e Retenção](https://github.com/BrendonSantos7/projeto-rh-analytics)**  
*Como o RH pode parar de apenas relatar demissões e começar a prever e evitar perdas financeiras?*  
Neste projeto, desenvolvi uma solução completa para identificar as causas raízes da evasão de talentos de uma empresa.
- **🔹 O Desafio:** Consolidar 7 bases de dados distintas e caóticas de RH para criar uma visão unificada e estratégica do capital humano.
- **💡 A Solução:** Arquitetura de dados em *Star Schema*, uso massivo de inteligência de tempo em DAX (para prever picos de saída) e prototipação de UI/UX aplicada no Figma para garantir uma navegação intuitiva.
- **🚀 O Impacto:** O painel traduziu dados demográficos em dinheiro, mapeando um impacto financeiro crítico de **R$ 3,90 milhões** em saídas voluntárias, permitindo aos gestores ações corretivas baseadas em dados, não em intuição.

[![Dashboard People Analytics](https://github.com/BrendonSantos7/projeto-rh-analytics/raw/main/recursos_visuais/prints_do_painel/print_visao_geral.png)](https://app.powerbi.com/view?r=eyJrIjoiZTM1MTIyYTAtMmU0ZC00ZWZkLWI1MDAtMDJjN2ViYjUxMTdhIiwidCI6ImFkOGRhY2IwLWU2OTgtNDJkZC04ODY2LWFkYWRkZTQ3MTEwZCJ9)  
🔗 **[Ver Repositório no GitHub](https://github.com/BrendonSantos7/projeto-rh-analytics)** · 📊 **[Acessar Dashboard Interativo](https://app.powerbi.com/view?r=eyJrIjoiZTM1MTIyYTAtMmU0ZC00ZWZkLWI1MDAtMDJjN2ViYjUxMTdhIiwidCI6ImFkOGRhY2IwLWU2OTgtNDJkZC04ODY2LWFkYWRkZTQ3MTEwZCJ9)**

<br>
<br>

**[💰 Solução End-to-End de Analytics — Finanças Corporativas/Pessoais](https://github.com/BrendonSantos7/Controle-Financeiro)**  
*Um projeto que comprova domínio de ponta a ponta: desde a modelagem física em banco de dados relacional até a entrega de valor visual.*  
- **🔹 O Desafio:** Eliminar a dependência de planilhas manuais isoladas e criar um ecossistema de dados estruturado, seguro e escalável para análises financeiras.
- **💡 A Solução:** Criação de um Data Warehouse local utilizando **SQL Server**, aplicando regras de negócio direto no banco, modelagem dimensional e, por fim, conexão direta e segura com o Power BI para a camada de visualização de KPIs.
- **🚀 O Impacto:** O dashboard oferece clareza instantânea sobre a saúde do caixa, cruzando receitas, despesas e saldo com análises de tendências temporais, provando capacidade de arquitetar soluções robustas que integram Back-end (Banco de dados) e Front-end (BI).

[![Dashboard Controle Financeiro](https://github.com/BrendonSantos7/Controle-Financeiro/raw/main/imagens/Dashboard.png?raw=true)](https://app.powerbi.com/view?r=eyJrIjoiNjk3NWI0NTgtNDJkYy00ZTk4LWIwZWQtZDkxMWFjM2JiYjY2IiwidCI6ImFkOGRhY2IwLWU2OTgtNDJkZC04ODY2LWFkYWRkZTQ3MTEwZCJ9)  
🔗 **[Ver Repositório no GitHub](https://github.com/BrendonSantos7/Controle-Financeiro)** · 📊 **[Acessar Dashboard Interativo](https://app.powerbi.com/view?r=eyJrIjoiNjk3NWI0NTgtNDJkYy00ZTk4LWIwZWQtZDkxMWFjM2JiYjY2IiwidCI6ImFkOGRhY2IwLWU2OTgtNDJkZC04ODY2LWFkYWRkZTQ3MTEwZCJ9)**

---
<p align="center"><i>Novos projetos com foco em Python e SQL Avançado sendo construídos. Portfólio em constante evolução. 🚧</i></p>
